...
export async function POST(req: NextRequest) {
    try {
        ...
        if (userPrisma.promoCodes.includes(code)) return NextResponse.json({ cStatus: 421, msg: `You already claimed this promo code.` }, { status: 400 });

        const promoCodePrisma = await getPromoCode(code);
